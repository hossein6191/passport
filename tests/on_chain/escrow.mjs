...
const c = createClient({ chain: NETWORK, account: buyer }); const cs = createClient({ chain: NETWORK, account: stranger }); const rd = createClient({ chain: NETWORK });
...
const wait = async (tx) => { for (let i = 0; i < 60; i++) { await new Promise((r) => setTimeout(r, 4000)); const t = await rpc("eth_getTransactionByHash", [tx]); if (t?.status === "ACCEPTED" || t?.status === "FINALIZED") { const lr = t.consensus_data?.leader_receipt, one = Array.isArray(lr) ? lr[0] : lr; let msg = ""; try { msg = Buffer.from(one.result, "base64").toString("utf8").replace(/[^\x20-\x7e]/g, " ").trim(); } catch (e) {} let j = null; const b = msg.indexOf("{"); if (b !== -1) { try { j = JSON.parse(msg.slice(b)); } catch (e) {} } return { msg, j, exec: one?.execution_result }; } } return { msg: "TIMEOUT" }; };
const deployFor = async (agent, operator, endpoint) => { const dh = await c.deployContract({ code, args: [REGISTER, agent, CLAIM, operator, endpoint], fees: await deployFees(c) }); return (await c.waitForTransactionReceipt({ hash: dh, waitUntil: "decided", retries: 40, interval: 4000, fullTransaction: true }))?.data?.contract_address; };
// a call that carries value is quoted with the default preset: the simulation does not carry the value
const send = async (E, client, fn, args = [], value) => wait(await client.writeContract({ address: E, functionName: fn, args, ...(value ? { value } : {}), fees: value ? await deployFees(client) : await feesFor(client, E, fn, args) }));
...
if (process.env.OPERATOR_KEY) {
  const co = createClient({ chain: NETWORK, account: createAccount(process.env.OPERATOR_KEY) });
